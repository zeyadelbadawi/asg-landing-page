...
export default function PartnersSection() {
  ...
  return (
    <section className="relative py-28 px-6 overflow-hidden" style={{ background: 'linear-gradient(180deg, #F8FAFC 0%, #FFFFFF 50%, #F8FAFC 100%)' }}>
      ...
      <div className="relative mb-5">
        ...
        <div className="flex animate-scroll-left gap-5 w-max">
          {[...row1, ...row1].map((p, i) => (
            <div
              key={`r1-${i}`}
              className="flex-shrink-0 px-8 py-5 rounded-2xl bg-white border border-[#1e2455]/[0.04] flex items-center justify-center min-w-[200px] hover:border-[#5AA4B1]/20 transition-all duration-300"
              style={{ boxShadow: '0 2px 20px rgba(30,36,85,0.03)' }}
            >
              <span className="text-sm font-semibold text-[#1e2455]/60 whitespace-nowrap tracking-wide">{p}</span>
            </div>
          ))}
        </div>
      </div>
